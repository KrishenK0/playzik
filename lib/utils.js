...
export function reqSong(googleId, videoId, minimal = false) {
    return new Promise((resolve, reject) => {
        if (googleId) {
            ...
            request.post(`https://music.youtube.com/youtubei/v1/playder?alt=json&key=AIzaSyC9XL3ZjWddXya6X74dJoCTL-WEYFDNX30`)
                .set(ytheader)
                .set({ 'X-Goog-Visitor-Id': googleId })
                .send({
                    'videoId': videoId,
                ...
                .then(async response => {
                    try {
                        if (response.statusCode !== 200) reject(reponse)
                        ...
                        reject({ code: 400, error: error.toString() });
                    }
                }).catch(error => {
