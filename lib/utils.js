...
export function getURLVideoID(id) {
    // const proxy = 'https://51.159.115.233:3128';
    // const proxy = 'http://163.172.85.160:9741';
    // const agent = HttpsProxyAgent(proxy);

    return ytdl.getInfo(id, { quality: 'highestaudio', /* requestOptions: { agent } */ })
        .then(info => {
        ...
        if (googleId) {
            ...
            request.post(`https://music.youtube.com/youtubei/v1/player?alt=json&key=AIzaSyC9XL3ZjWddXya6X74dJoCTL-WEYFDNX30`)
                ...
                .end(async (error, response) => {
                    ...
                    let infoPayload = JSON.parse(response.text).videoDetails;
